...
if test -n "$ALATTY_SI_RUN_COMMAND_AT_STARTUP"
    printf '\e]2;%s\a' (string replace -ra '[\x00-\x1F\x7F]' '' -- "$ALATTY_SI_RUN_COMMAND_AT_STARTUP")
    set --local _krcs "$ALATTY_SI_RUN_COMMAND_AT_STARTUP"
    set --erase ALATTY_SI_RUN_COMMAND_AT_STARTUP
    eval "$_krcs"
end
...
function __ksi_schedule --on-event fish_prompt -d "Setup alatty integration after other scripts have run, we hope"
    ...
    if test -n "$SSH_KITTEN_ALATTY_DIR"
        if not contains -- "$SSH_KITTEN_ALATTY_DIR" "$PATH"
            if not type kitten 2> /dev/null > /dev/null
                set -gx PATH "$PATH" "$SSH_KITTEN_ALATTY_DIR"
            end
        end
        set --erase SSH_KITTEN_ALATTY_DIR
    end
