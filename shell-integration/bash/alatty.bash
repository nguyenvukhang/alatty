...
_ksi_main() {
    ...
    builtin unset ALATTY_SHELL_INTEGRATION
    if [[ -n "$SSH_KITTEN_ALATTY_DIR" ]]; then
        if [[ ! "$PATH" =~ (^|:)${SSH_KITTEN_ALATTY_DIR}(:|$) ]] && [[ -z "$(builtin command -v kitten)" ]]; then
            builtin export PATH="${PATH}:${SSH_KITTEN_ALATTY_DIR}"
        fi
        builtin unset SSH_KITTEN_ALATTY_DIR
    fi
    builtin local krcs="$ALATTY_SI_RUN_COMMAND_AT_STARTUP"
    builtin unset ALATTY_SI_RUN_COMMAND_AT_STARTUP
