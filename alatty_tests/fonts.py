...
from alatty.fonts.render import coalesce_symbol_maps, create_face, render_string, setup_for_testing, shape_string
...
def testing_font_data(name):
    return read_alatty_resource(name, __name__.rpartition('.')[0])
...
def test_chars(chars: str = '╌', sz: int = 128) -> None:
    # alatty +runpy "from alatty.fonts.box_drawing import test_chars; test_chars('XXX')"
    from alatty.fast_data_types import concat_cells, render_box_char, set_send_sprite_to_gpu
    from alatty.fonts.render import display_bitmap, setup_for_testing
    if not chars:
        import sys
        chars = sys.argv[-1]
