...
from alatty.constants import is_macos, alatty_exe, read_alatty_resource
...
from alatty.utils import is_ok_to_read_image_file, is_path_in_temp_dir, sanitize_title, sanitize_url_for_display_to_user, shlex_split, shlex_split_with_positions
...
class TestDataTypes(BaseTest):
            ...
            for x, ex in {
                ...
                if ex is None:
                    ...
                    with self.assertRaises(ex, msg=x):
                        makedirs(q)
        saved = {x: os.environ.get(x) for x in 'ALATTY_CONFIG_DIRECTORY XDG_CONFIG_DIRS XDG_CONFIG_HOME'.split()}
        try:
            ...
            with tempfile.TemporaryDirectory() as tdir:
                with open(tdir + '/macos-launch-services-cmdline', 'w') as f:
                    print('alatty +runpy "import sys; print(sys.argv[-1])"', file=f)
                    print('next-line', file=f)
                    print()
                if is_macos:
                    env = os.environ.copy()
                    env['ALATTY_CONFIG_DIRECTORY'] = tdir
                    env['ALATTY_LAUNCHED_BY_LAUNCH_SERVICES'] = '1'
                    cp = subprocess.run([alatty_exe(), '+runpy', 'import json, sys; print(json.dumps(sys.argv))'], env=env, stdout=subprocess.PIPE)
                    actual = cp.stdout.strip().decode()
                    if cp.returncode != 0:
                        print(actual)
                        raise AssertionError(f'alatty +runpy failed with return code: {cp.returncode}')
                    self.ae('next-line', actual)
                os.makedirs(tdir + '/good/alatty')
                open(tdir + '/good/alatty/alatty.conf', 'w').close()
                data = os.urandom(32879)
                with open(tdir + '/f', 'wb') as f:
                    f.write(data)
                self.ae(data, read_file(f.name))
                for x in (
                    (f'ALATTY_CONFIG_DIRECTORY={tdir}', f'{tdir}'),
                    (f'XDG_CONFIG_HOME={tdir}/good', f'{tdir}/good/alatty'),
                    (f'XDG_CONFIG_DIRS={tdir}:{tdir}/good', f'{tdir}/good/alatty'),
                    (f'XDG_CONFIG_DIRS={tdir}:{tdir}/bad:{tdir}/f', f'{dot_config}/alatty'),
                    (f'{dot_config}/alatty',),
                ):
                    for k in saved:
    ...
    def test_split_into_graphemes(self):
        ...
        for i, test in enumerate(json.loads(read_alatty_resource('GraphemeBreakTest.json', __name__.rpartition('.')[0]))):
            expected = test['data']
            text = ''.join(expected)
