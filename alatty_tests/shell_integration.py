...
''') as pty:
        ...
        with self.run_shell(rc='PS1=XXX', extra_env={'ALATTY_SI_RUN_COMMAND_AT_STARTUP': 'echo pre-start'}) as pty:
    ...
    def test_fish_integration(self):
        ...
        with self.run_shell(
            shell='fish',
            extra_env={'ALATTY_SI_RUN_COMMAND_AT_STARTUP': 'echo XXX'},
            rc=f'''
set -g fish_greeting
...
''') as pty:
    ...
    ''') as pty:
        ...
        with self.run_shell(shell='bash', rc='PS1=XXX', extra_env={'ALATTY_SI_RUN_COMMAND_AT_STARTUP': 'echo pre-start'}) as pty:
            pty.wait_till(lambda: 'XXX' in pty.screen_contents())
            self.assertIn('pre-start', pty.screen_contents())
            self.assertTrue(pty.screen_contents().startswith('pre-start'))
