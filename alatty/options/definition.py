...
from alatty.options.utils import pointer_shape_names
...
By default alatty renders gaps in underlines when they overlap with descenders
...
between the foreground and background color, below which alatty will override the foreground color.
...
:link:`alatty-scrollback.nvim <https://github.com/mikesmithgh/alatty-scrollback.nvim>`
...
:code:`<unhide-threshold>`
    ...
    For example, if :code:`<unhide-threshold>` is 40 and :code:`<unhide-wait>` is 2.5, when alatty
...
that alatty will not apply any underline styling to hyperlinks. Note that the value of :code:`always`
...
is changed, so there, copying to clipboard in a non-alatty application will not clear
...
with respect to the alatty config directory.
'''
    )
...
new instances of alatty will have the same position as the previous instance.
...
manager supports it. It never works on Wayland. See also :option:`alatty --position` to
specify the position when launching alatty.
'''
    )
...
:code:`session_name`
    The name of the alatty session file from which this tab was created, if any.
:code:`active_session_name`
    The name of the alatty session file from which the active window in this tab was created, if any.
:code:`num_windows`
...
:code:`custom`
    ...
    the alatty config directory. The function will be passed a dictionary of data, the same data that
    ...
    Any print statements in the :code:`draw_title()` will print to the STDOUT of the alatty process, useful
    for debugging.
...
:code:`echo hello world >&$ALATTY_STDIO_FORWARDED` in a shell will print
to the parent alatty's STDOUT. Sets the :code:`ALATTY_STDIO_FORWARDED=fdnum`
...
To setup shortcuts to multiply/divide the font size::
    ...
    map alatty_mod+f6 change_font_size all *2.0
    map alatty_mod+f6 change_font_size all /2.0
''')  # }}}
