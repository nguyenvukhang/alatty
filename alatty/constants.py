...
alatty_run_data: dict[str, Any] = getattr(sys, 'alatty_run_data', {})
launched_by_launch_services = alatty_run_data.get('launched_by_launch_services', False)
is_quick_access_terminal_app = alatty_run_data.get('is_quick_access_terminal_app', False)
unserialize_launch_flag = 'alatty-unserialize-data='

if getattr(sys, 'frozen', False):
    extensions_dir: str = alatty_run_data['extensions_dir']
...
@run_once
def alatty_exe() -> str:
    rpath = alatty_run_data.get('bundle_exe_dir')
    if not rpath:
...
def _get_config_dir() -> str:
    cdir = alatty_run_data.get('config_dir', '')
    if cdir:
        return str(cdir)
    import atexit
    import tempfile
...
def running_in_alatty(set_val: bool | None = None) -> bool:
...
def local_docs() -> str:
    d = os.path.dirname
    base = d(d(alatty_exe()))
    from_source = alatty_run_data.get('from_source')
