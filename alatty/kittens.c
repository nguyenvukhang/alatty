...
parse_input_from_terminal(PyObject *self UNUSED, PyObject *args) {
    ...
    PyObject *uo, *text_callback, *dcs_callback, *csi_callback, *osc_callback, *pm_callback, *callback;
    int inbp = 0;
    if (!PyArg_ParseTuple(args, "OOOOOUp", &text_callback, &dcs_callback, &csi_callback, &osc_callback, &pm_callback, &uo, &inbp)) return NULL;
