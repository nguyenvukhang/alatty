...
class Boss:
    ...
    def quit(self, *args: Any) -> None:
        ...
        if current_application_quit_request() == CLOSE_BEING_CONFIRMED:
            ...
            return
        msg = msg or _('It has {} windows.').format(num)
        w = self.confirm(_('Are you sure you want to quit alatty?') + ' '  + msg, self.handle_quit_confirmation, window=active_window, title=_('Quit alatty?'))
    ...
    def run_kitten_with_metadata(
        ...
        if w is not None and tab is not None:
            ...
            if is_wrapped:
                ...
                if w is not None:
                    env['ALATTY_BASIC_COLORS'] = json.dumps(w.screen.color_profile.basic_colors())
            else:
    ...
    @ac('session', 'Save the current alatty state as a session file. See :ref:`save_as_session`.')
    ...
    @ac('session', '''
        ...
        Examples::
            ...
            map f1 close_session "/path/to/session/file.alatty-session"
    ''')
