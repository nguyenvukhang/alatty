...
# as it is used to generate C code when building alatty
...
try:
    from alatty.constants import appname, is_macos
except ImportError:
    is_macos = 'darwin' in sys.platform.lower()
try:
    from alatty.utils import shlex_split as ksplit
...
def generate_c_parsers() -> Iterator[str]:
    ...
    yield from generate_c_parser_for('alatty', alatty_options_spec())
    yield ''
    yield ''
...
# alatty CLI spec {{{
grab_keyboard_docs = """\
Grab the keyboard. This means global shortcuts defined in the OS will be passed to alatty instead. Useful if
...
def alatty_options_spec() -> str:
    if not hasattr(alatty_options_spec, 'ans'):
        OPTIONS = '''
--class --app-id
dest=cls
default={appname}
...
--position
The position, for example 10x20, on screen at which to place the first alatty OS Window.
This may or may not work depending on the policies of the desktop
environment/window manager. It never works on Wayland.
See also :opt:`remember_window_position` to have alatty automatically try
...
!
'''
        setattr(alatty_options_spec, 'ans', OPTIONS.format(
    ...
    ans: str = getattr(alatty_options_spec, 'ans')
    return ans
# }}}
