...
run_embedded(RunData *run_data) {
...
#endif
    safe_snprintf(extensions_dir_full, PATH_MAX, "%s/%s/alatty-extensions", run_data->exe_dir, python_relpath);
    ...
    bypy_initialize_interpreter(
            L"alatty", python_home, L"alatty_main", extensions_dir, run_data->cli_spec->original_argc, run_data->cli_spec->original_argv);
...
read_exe_path(char *exe, size_t buf_sz) {
    ...
    while (token != NULL) {
        char q[PATH_MAX + 1] = {0};
        safe_snprintf(q, PATH_MAX, "%s/alatty", token);
...
static bool
parse_and_check_alatty_cli(CLISpec *cli_spec, int argc, char **argv) {
    parse_cli_for_alatty(cli_spec, argc, argv);
    if (cli_spec->errmsg) {
        fprintf(stderr, "%s\n", cli_spec->errmsg);
#ifdef __APPLE__
...
#define swap_cli_spec \
    ...
    if (instance_group_prefix == NULL) {
        ...
        if (offset) {
            if (!parse_and_check_alatty_cli(&subcommand_cli_spec, cli_spec->original_argc - offset, cli_spec->original_argv + offset)) exit(1);
            swap_cli_spec;
...
#undef reopen_or_fail
            if (fork() != 0) {
                ...
                // see: https://github.com/kovidgoyal/alatty/issues/8680
                char buf[4];
...
main(int argc_, char *argv_[], char* envp[]) {
    ...
    is_quick_access_terminal = endswith(exe, "/alatty-quick-access");
    if (getenv("ALATTY_LAUNCHED_BY_LAUNCH_SERVICES")) {
        launched_by_launch_services = true;
        unsetenv("ALATTY_LAUNCHED_BY_LAUNCH_SERVICES");
    ...
    bool ok = parse_and_check_alatty_cli(&cli_spec, argva.count, argva.argv);
    if (!ok) return 1;
    if (!handle_fast_commandline_called) handle_fast_commandline(&cli_spec, NULL);
    int ret=0;
    char lib[PATH_MAX+1] = {0};
    if (ALATTY_LIB_PATH[0] == '/') {
        safe_snprintf(lib, PATH_MAX, "%s", ALATTY_LIB_PATH);
    } else {
        safe_snprintf(lib, PATH_MAX, "%s/%s", exe_dir, ALATTY_LIB_PATH);
    }
    RunData run_data = {
