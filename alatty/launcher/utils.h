...
is_dir_ok_for_config(char *q) {
    size_t len = strlen(q);
    memcpy(q + len, "/alatty", sizeof("/alatty"));
    len += sizeof("/alatty") - 1;
    memcpy(q + len, "/alatty.conf", sizeof("/alatty.conf"));
...
#define expand(x, dest, sz) { expand_tilde(x, buf1, sizeof(buf1)); lexical_absolute_path(buf1, dest, sz); }
    q = getenv("ALATTY_CONFIG_DIRECTORY"); if (q && q[0]) { expand(q, output, outputsz); return true; }
#define check_and_ret(x) if (x && x[0]) { expand(x, output, outputsz); if (is_dir_ok_for_config(output)) return true; }
    ...
    safe_snprintf(output, outputsz, "%s/alatty", buf2);
