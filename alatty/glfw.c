...
#define glfw_failure { \
        PyErr_Format(PyExc_OSError, "Failed to create GLFWwindow. This usually happens because of old/broken OpenGL drivers. alatty requires working OpenGL %d.%d drivers.", OPENGL_REQUIRED_VERSION_MAJOR, OPENGL_REQUIRED_VERSION_MINOR); \
        return NULL; }
...
ring_audio_bell(OSWindow *w) {
    ...
    if (OPT(bell_path)) play_canberra_sound(OPT(bell_path), "alatty bell", true, "event", OPT(bell_theme));
        ...
        if (!global_state.is_wayland || !glfwWaylandBeep(w ? w->handle : NULL)) play_canberra_sound(
                "bell", "alatty bell", false, "event", OPT(bell_theme));
    }
#endif
}
