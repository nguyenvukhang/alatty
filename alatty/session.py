...
def session_arg_to_name(session_arg: str) -> str:
    ...
    if session_name.rpartition('.')[2] in ('session', 'alatty-session', 'alatty_session'):
...
def parse_goto_session_cmdline(args: list[str]) -> tuple[GotoSessionOptions, list[str]]:
    from alatty.cli import cached_parse_cmdline
...
Save the current state of alatty as a session file for easy re-use. If the path at which to save the session
file is not specified, alatty will prompt you for one. If the path is :code:`.` it will save the session
...
def save_as_session(boss: BossType, cmdline: Sequence[str]) -> None:
    ...
    if path:
        ...
        boss.get_save_filepath(_(
            'Enter the path at which to save the session, usually session files are given the .alatty-session file extension'),
