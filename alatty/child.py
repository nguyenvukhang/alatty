...
if is_macos:
    from alatty.fast_data_types import abspath_of_process as _abspath_of_process
...
def process_env(env: Mapping[str, str] | None = None) -> dict[str, str]:
    ...
    ans.pop('ALATTY_SI_RUN_COMMAND_AT_STARTUP', None)
    return ans
    ...
    def get_final_env(self) -> tuple[dict[str, str], bool]:
        ...
        if self.startup_command_via_shell_integration:
            if isinstance(self.startup_command_via_shell_integration, str):
                env['ALATTY_SI_RUN_COMMAND_AT_STARTUP'] = self.startup_command_via_shell_integration
            else:
                from .shell_integration import join
                scmd = self.argv or resolved_shell(fast_data_types.get_options())
                try:
                    env['ALATTY_SI_RUN_COMMAND_AT_STARTUP'] = join(scmd[0], self.startup_command_via_shell_integration)
                except Exception:
