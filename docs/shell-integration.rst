...
``redraw=0``
    this tells alatty that the shell will not redraw the prompt on
    resize so it should not erase it

``special_key=1``
    this tells alatty to use a special key instead of arrow keys
...
``k=s``
    this tells alatty that the secondary (PS2) prompt is starting at the
    current line.

``click_events=1``
    this tells alatty that the shell is capable of handling
