...
These are a few small protocol extensions alatty implements, primarily for use
...
save/restore. alatty extends this protocol to specify that when no modes are
...
There is no way to reset one and not the other. alatty uses 221 and 222 to reset
...
alatty extends the SGR Pixel mouse reporting protocol created by xterm to
...
alatty specific private escape codes
---------------------------------------

These are a family of escape codes used by alatty for various things including
remote control. They are all DCS (Device Control String) escape codes starting
with ``\x1b P @ alatty-`` (ignoring spaces present for clarity).
