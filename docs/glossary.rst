...
.. envvar:: ALATTY_SI_RUN_COMMAND_AT_STARTUP

   Set this to an expression that the alatty shell integration scripts will
   ``eval`` after the shell is started. Note that this environment variable
   is ignored when present in the environment in which alatty itself is launched
