...
You can use alatty's remote control features inside the main() function of a
kitten, even without enabling remote control. This is useful if you want to
probe alatty for more information before presenting some UI to the user or if
you want the user to be able to control alatty from within your kitten's UI
rather than after it has finished running. To enable it, simply tell alatty your kitten
...
:code:`allow_remote_control=True` tells alatty to run this kitten with remote
...
`alatty-tab-switcher <https://github.com/OsiPog/alatty-tab-switcher>`__
    Fuzzy finder for alatty tabs with previews

`gattino <https://github.com/salvozappa/gattino>`__
    Integrate alatty with an LLM to convert plain language prompts into shell commands.
