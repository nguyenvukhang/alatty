...
.. note::
   ...
   ``include`` it into alatty.conf. The include should be placed after the
...
You can have alatty automatically change its color theme when the OS switches
...
a theme for each of the three modes. Then, once you restart alatty, it will
...
alatty config directory. When these files exist, alatty queries the OS for its color scheme
...
even those specified using the :option:`alatty --override` command line flag.
alatty will also automatically change colors when the OS color scheme changes,
...
.. note::
   ...
   in which case GNOME will report the color scheme as light and alatty will use
